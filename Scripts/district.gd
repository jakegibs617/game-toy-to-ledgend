...
func _smoke_playtest_metrics() -> void:
	...
	assert((balance["caps"] as Dictionary).has("calligraphy"))
	assert(is_equal_approx(float((balance["caps"] as Dictionary)["calligraphy"]["repMultiplier"]), 1.25))
	assert(int((balance["caps"] as Dictionary)["calligraphy"]["paintDelta"]) == 1)
	assert(is_equal_approx(float((balance["crew"] as Dictionary).get("moraleFactorMin", 0.0)), 0.9))
	assert(is_equal_approx(float((balance["crew"] as Dictionary).get("moraleFactorNeutral", 0.0)), 1.0))
	assert(is_equal_approx(float((balance["crew"] as Dictionary).get("moraleFactorMax", 0.0)), 1.1))
...
func _smoke_balance_invariants() -> void:
	...
	assert(SupplyManager.cap_def("calligraphy").get("repMultiplier", 1.0) <= 1.25)
	assert(CrewManager.morale_factor_for(CrewManager.MAX_MORALE) <= 1.1)
